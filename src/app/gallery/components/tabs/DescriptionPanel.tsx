  ...
  return (
    <div className='flex flex-col h-full w-full bg-neutral-50/50 rounded-3xl border border-neutral-100 overflow-hidden'>
      <div className='flex-1 overflow-auto custom-scrollbar p-8 space-y-10'>
        ...
        <section className='animate-in fade-in slide-in-from-bottom-3 duration-700'>
          <h4 className='text-[11px] font-bold text-neutral-400 uppercase tracking-[0.2em] mb-5 flex items-center gap-2'>
            <span className='w-4 h-px bg-neutral-200'></span> Key Features
          </h4>
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            {(item.features || []).map((feature, idx) => (
              ...
              >
                <span className='shrink-0 w-6 h-6 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center text-[10px] font-black'>
                  {idx + 1}
                </span>
        ...
        <section className='animate-in fade-in slide-in-from-bottom-4 duration-1000'>
          <h4 className='text-[11px] font-bold text-neutral-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2'>
            <span className='w-4 h-px bg-neutral-200'></span> Tech Stack
          </h4>
      ...
      {/* デザインの統一感のため、コードパネルのフッターと同じ高さの
        装飾バーを付けても良いでしょう（任意）
      */}
      <div className='px-5 py-2 bg-white/50 border-t border-neutral-100 flex justify-end items-center shrink-0'>
