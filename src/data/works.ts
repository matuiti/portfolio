...
export const ALL_WORKS: Work[] = [
  {
    ...
    images: ["/assets/images/works/cms-template-detail.jpg"],
    summary:
    ...
    features:
      ...
      "・【高度なUI】ブレイクポイントごとに細やかな制御を行うレスポンシブヘッダー\n" +
      "・【検索UI】多階層アコーディオンUIで情報量とコンパクト性を両立した大型モーダル検索メニュー\n" +
      "・【UX】スクロール追従型で検索・アカウント・人気記事等の拡張パネルを展開するボトムメニュー\n" +
      "・【コミュニティ機能】返信機能、本人判定UI、インタラクション（全文表示等）を備えたコメントエリア\n" +
      "・【スライダー】Swiperを用い、縦/横/全画面/複数型/自動スクロール/中央強調など多様なパターンを実装\n" +
      "・【汎用性】テーマ別スタイルに対応したサイドメニューや多機能カード（複数バリエーション）",
    points:
      "・【CSS設計】ルートクラスによるテーマ切り替え機構と、Tailwindのユーティリティ×カスタムクラスの適切な使い分けによる高再利用性の実現\n" +
      "・【品質管理】SEO・アクセシビリティの徹底、プロジェクト専用フォーマット設定によるコード一貫性の確保、UI/UXに関する細部へのフィードバック対応\n" +
      "・【付加価値】レビュー用ポータルページの作成、引き継ぎ用ドキュメント（コードリーディング支援）作成、新規ブレイクポイント追加対応、CMS設定に応じたUI切り替え対応\n" +
      "・【チーム連携】各分野のプロとの協働。1人で半日悩むより10分のMTGで解決する迅速な課題解決と、互いのミスを補完し合う品質追求姿勢を共有\n" +
      "・【継続改善】プロジェクト後、EJSによるHTMLモジュール化やPlaywrightによるテストコード導入など、開発環境のリファクタリングを継続実施",
    role: "コーディング, 見積もり",
    duration: "3ヶ月",
    disclosureLevel: "NDA",
  },
  {
    ...
    background:
      "静的なHTMLサイトをWordPress化。デザイナーと密に連携し、サイト全体の構造を共有しながら、単なる機能追加に留まらない実用的なポートフォリオサイトを構築しました。",
    features:
  ...
  },
  {
    ...
    features:
      "・【下層ページ刷新】タブメニューによるコンテンツとURLの動的切り替え、折りたたみ収納機能の実装\n・【検索UI改善】PC/SPそれぞれに最適化した検索窓デザイン、ヘッダー（検索窓）のスクロール追従化\n・【ユーザビリティ】サイドメニューの開閉状態制御、検索結果のタグ視認性向上\n・【運用の効率化】モバイルメニューやログインUIの調整、既存の表への項目追加とバグ対応",
    points:
  ...
  },
  {
    ...
    points:
      "・情報不足を先読みして他のコーダーへ積極的に相談し、大規模案件特有の手戻りを未然に防止\n・既存のSass/CSS設計を読み解き、不要な影響を与えないよう慎重にスコープを管理して実装。この確実な仕事が評価され、完遂直後に別件のご相談をいただきました。",
    role: "コーディング",
    duration: "2ヶ月",
    disclosureLevel: "NDA",
  },
